...
LD_FLAGS                    := "-w -X github.com/fi-ts/gardener-extension-accounting/pkg/version.Version=$(IMAGE_TAG)"
VERIFY                      := true
...
.PHONY: build
build:
	go build -ldflags $(LD_FLAGS) -tags netgo ./cmd/gardener-extension-accounting
...
docker-image:
	@docker build --no-cache \
		...
		--tag $(IMAGE_PREFIX)/gardener-extension-accounting:$(IMAGE_TAG) \
		--file Dockerfile --memory 6g .

.PHONY: docker-push
docker-push:
	@docker push $(IMAGE_PREFIX)/gardener-extension-accounting:$(IMAGE_TAG)
...
push-to-gardener-local:
	CGO_ENABLED=1 go build \
		...
		-o bin/gardener-extension-accounting \
		./cmd/gardener-extension-accounting
